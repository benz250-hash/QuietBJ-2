...
def build_summary_line(signals: list[dict[str, Any]]) -> str:
    ...
    negatives = [x for x in signals if int(x.get("score_delta", 0)) < 0]
    ordered = sorted(negatives, key=lambda x: abs(int(x.get("score_delta", 0))), reverse=True)
...
def road_point_for_engine_result(
    ...
    if not target_point:
        return None
    distance_hint = _to_int_distance(road_result.evidence.get("distance_m"))
    roads = list((regeo or {}).get("roads", []) or [])
    candidates: list[tuple[float, tuple[float, float]]] = []
    for road in roads:
        parsed = parse_location_text(str(road.get("location", "")).strip())
        if not parsed:
            continue
        actual = _distance_between_gcj_points_m(target_point, parsed)
        gap = actual if distance_hint is None else abs(actual - distance_hint)
        candidates.append((gap, parsed))
    if not candidates:
        return None
    candidates.sort(key=lambda x: x[0])
...
def build_engine_results(
    ...
    locator_conf = str(community_row.get("_locator_confidence", "")).strip() or "中"
    results.append(engine_zone_result(selected_zone_code, locator_confidence=locator_conf))
    ...
    if str(community_row.get("_override_zone_type", "")).strip():
        notes = str(community_row.get("_override_notes", "")).strip() or "命中人工校正规则。"
        results.append(override_result("人工校正", 0, notes))
...
def parse_geocode_result(query: str, community_repo: CommunityRepository, amap: AMapProvider) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any] | None, str, dict[str, Any] | None]:
    ...
    tips = cached_input_tips(amap.api_key, query) if amap.enabled() else []
...
def compute_position_result(
    zone_options: list[dict[str, Any]],
    community_row: dict[str, Any],
    ...
    result = result_dict_from_breakdown(
        breakdown=breakdown,
        zone_name=selected_name,
        zone_description=zone_row.get("description", "按当前楼栋位置调整"),
    )
    return result
...
def render_position_card(result: dict[str, Any], zone_labels: list[str], zone_key: str) -> None:
    with st.container(border=True):
        ...
        summary = (
            ...
            f"密度调整 {result['density_adjustment']:+d}。"
        )
...
def main() -> None:
    ...
    selected_zone_row = next((z for z in zone_options if str(z.get("zone_name", "")) == selected_name), zone_options[default_idx])
    selected_zone_code = str(selected_zone_row.get("zone_code", "")).strip() or "central"
    engine_results = build_engine_results(
        community_row=community_row,
        building_location_text=building_location_text,
        regeo=regeo,
        poi_results=poi_results,
        selected_zone_code=selected_zone_code,
    )
    result = compute_position_result(zone_options, community_row, score_engine, engine_results, selected_name)
    noise_summary = build_noise_summary_from_breakdown(result["breakdown"])
